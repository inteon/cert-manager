...
func (o *versionChecker) Version(ctx context.Context) (string, error) {
	...
	if version, err = o.extractVersionFromClusterRole(ctx, certManagerLabelSelector); shouldReturn(err) {
		return version, err
	}

	return o.extractVersionFromClusterRole(ctx, certManagerOldLabelSelector)
}
