		...
		t.Run(tc.name, func(t *testing.T) {
			{
				...
				tc.raw.FullBytes = fullBytes

				if !reflect.DeepEqual(rawValue, tc.raw) {
					t.Errorf("Expected rawValue: %v, got: %v", tc.raw, rawValue)
				}
			}
...
func TestIsIA5String(t *testing.T) {
	...
	for _, ia5String := range ia5Strings {
		isIA5 := isIA5String(ia5String)

		if !isIA5 {
			t.Errorf("Expected IA5 string %q, got: %v", ia5String, isIA5)
		}
	}
	...
	for _, nonIA5String := range nonIA5Strings {
		isIA5 := isIA5String(nonIA5String)

		if isIA5 {
			t.Errorf("Expected non-IA5 string %q, got: %v", nonIA5String, isIA5)
		}
	}
}
