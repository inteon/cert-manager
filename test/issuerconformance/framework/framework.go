...
import (
	...
	"issuerconformance/framework/helper"
...
func (f *Framework) BeforeEach(ctx context.Context) {
	f.beforeEachOnce.Do(func() {
		f.setup(ctx)
	})
}

func (f *Framework) setup(ctx context.Context) {
	if f.KubeClientConfig == nil {
		Fail("Framework.KubeClientConfig is nil!")
	}

	f.CleanupLabel = "cm-conformance-cleanup-" + e2eutil.RandStringRunes(10)

	var err error
	kubeConfig := f.KubeClientConfig
...
func (f *Framework) AfterEach(ctx context.Context) {
	if f.CRClient == nil {
		Fail("Framework.CRClient is nil!")
	}
