...
import (
	...
	"github.com/cert-manager/cert-manager/core-issuers-binary/controller/clusterissuers"
	dnsutil "github.com/cert-manager/cert-manager/core-issuers-binary/issuer/acme/dns/util"
...
func Run(opts *options.ControllerOptions, stopCh <-chan struct{}) error {
	...
	defer cancelContext()
	rootCtx = logf.NewContext(rootCtx, logf.Log, "core-issuers")
	...
	for n, fn := range controller.Known() {
		log := log.WithValues("core-issuers", n)
...
func startLeaderElection(ctx context.Context, opts *options.ControllerOptions, leaderElectionClient kubernetes.Interface, recorder record.EventRecorder, callbacks leaderelection.LeaderCallbacks) error {
	// Identity used to distinguish between multiple core-issuers instances
	id, err := os.Hostname()
	if err != nil {
		return fmt.Errorf("error getting hostname: %v", err)
	}

	lockName := "cert-manager-core-issuers"
	lc := resourcelock.ResourceLockConfig{
		Identity:      id + "-external-cert-manager-core-issuers",
